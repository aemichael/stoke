...
// TODO: Add an argument for user-defined values to hit
// For now, just use hard-coded cases: 0, 1, 0xFFF..F
const std::vector<uint64_t> mutation_targets = {0, 1, UINT64_MAX};
...
/** Make testcase that changes registers to match a mutation target */
CpuState mutate_to_target(CpuState cs, size_t num_registers, size_t target_index,
  ...
  for (size_t i = 0; i < num_registers; i++) {
    ...
    candidate.gp[reg_choice].get_fixed_quad(0) = mutation_targets[target_index];
    reg_choice = (reg_choice + 1) % 16;
